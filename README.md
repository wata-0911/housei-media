...
- 生成済み`planner_catalog_2026.json`、`plannerCatalog.ts`、`plannerHelpers.ts`とJSON Schemaをそのままコピーしています。再生成やIDの再推定は行っていません。監査済みの例外52開講（初回34 + cleanup 18）だけは`planner_manual_mapping_overrides_2026.json`の`manual_curated` ledgerを読込時に適用し、元のoffering名と科目identityを保持します。
